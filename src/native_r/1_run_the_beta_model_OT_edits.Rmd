...
library(R2jags)
library(ggplot2)
# library(rgdal)  #OT deprecated from CRAN 2023-10-16 (https://cran.r-project.org/web/packages/rgdal/index.html). Have replaced functionality with terra package: https://cran.r-project.org/web/packages/terra/index.html:
library(terra)
library(dplyr)
...
library(viridisLite)
library(swfscMisc)
# library(plotKML)  #OT removed from CRAN 2023-10-16 (https://cran.r-project.org/web/packages/plotKML/index.html). No use for demonstration
```

```{r initialize directories and read in data}
#set names for the directories where your data are stored
#OT have replaced hardcoded paths with my own wherever necessary
...
# home <- "/path/to/unzipped/directory/attached/to/email" e.g.:
# home <- "C:/Users/<username>/Desktop/indexing_Sully_explanation"
home <- "/Users/rt582/Desktop/indexing_Sully_explanation"

#OT coral_cover_directory unnecessary (data.csv is in home)
shapefiles_directory <- file.path(home, "shapefiles") #OT
output_directory <- file.path(home, "output") #OT
#OT make output directory if it doesn't exist
...
setwd(home) #OT
#coral cover data
# setwd(output_directory) #OT unnecessary switch
# setwd("C:/Users/Shannon/Desktop/Ecoregions/Coral cover/GitHub_code")  #OT unnecessary switch
...
# setwd(coral_cover_directory)  #OT unnecessary switch

# setwd(home) #OT unnecessary switch
Sys.sleep(1)
...
#OT No `coral_diversity_for_coral_cover.csv` provided in repository – using Coral_diversity.csv provided by R. van Woesik by email


#diversity data
# setwd(output_directory) #OT unnecessary
diversity<-read.csv(file=file.path("Coral_diversity.csv"), header=TRUE, sep=",")  #OT
# names(diversity)[1]<-"Ecoregion"  #OT
# diversity$Region<-diversity$Ecoregion #OT
# diversity<-diversity[order(diversity$Ecoregion),] #OT

```

```{r read in and format shapefiles of a world map, and ecoregion boundaries}
#OT – have had to switch to terra from sp since sp was deprecated in 2023

# wlrd.p <- readOGR(file.path(shapefiles_directory,'TM_WORLD_BORDERS_SIMPL_PC150.shp')) #OT replacing with terra
# ECO <- readOGR(file.path(shapefiles_directory,'ecoregion_exportPolygon.shp')) # ecoregions  #OT replacing with terra
...
#OT terra does the below assignments much more simply
# ecos_list<-c()
# for (i in 1:150){
...
#OT replacing loop above that filtered exterior rings (ringDir==1) to drop polygon holes
ECO <- terra::fillHoles(ECO)
ECO <- as(ECO, "Spatial")
```

```{r OT – this was orphaned code (not in a cell)}
...
# coral_cover_data$Longitude<-coral_cover_data$lon  #OT – this overwrites lat assignment which causes issues in line 179
...
#OT assign lat and lon again (current versions of packages eat the lat/lon columns)
coral_cover_data$lat<-coral_cover_data$Latitude.Degrees
coral_cover_data$lon<-coral_cover_data$Longitude.Degrees
```
...
#OT – since no diversity data is available, taking the pre-standardised diversity in data_for_maps.csv's 'diversity.standardized' column as truth
data_for_maps <- read.csv(file.path(home, "data_for_maps.csv"), header=TRUE)
...
#OT – plot a scatter of diversity for each ecoregion vs. the pre-standardised diversity in data_for_maps.csv's 'diversity.standardized' column to validate my previous exploration
ggplot(diversity, aes(x=SpeciesAccepted, y=diversity.standardized)) +
  geom_point() +
  geom_abline(slope=1, intercept=0) +
  labs(x="SpeciesAccepted from Coral_diversity.csv", y="diversity.standardized from data_for_maps.csv")
ggsave(file.path(output_directory, "diversity_scatter.png"))
#OT – 1:1 line shows that previous mapping method was correct
...
# sites_and_region_df <-left_join(sites_and_region_df,diversity[,c('ERName','diversity.standardized')],by='ERName') #OT – don't have this file
...
# fixing ecoregion assignment
region_diversity_df <- data %>% # N.B. Ecoregion.x == Ecoregion.y (hangover from df merge)
  dplyr::distinct(Ecoregion.x, diversity.standardized) %>%  # unique rows for each region/diversity combo
  dplyr::mutate(region = as.numeric(as.factor(Ecoregion.x))) %>%  # assign numeric region index
  dplyr::arrange(region)                                        # sort by the region index
# these diversity values are correct
```

```{r audit jags indexing — save CSV of data as passed to the model}

obs_site <- data$site
obs_ecoregion <- as.character(data$Ecoregion.x) 
obs_reef <- as.character(data$Reef_ID)

Nre_audit <- length(unique(obs_site))
paper_region_jags <- as.integer(
  as.factor(as.character(sites_and_region_df$region))
)
correct_by_site <- sites_and_region_df[order(sites_and_region_df$site), ]
region_true_by_site <- correct_by_site$region
eco_true_by_site <- correct_by_site$Ecoregion
div_true_by_site <- correct_by_site$diversity.standardized

diversity_vec <- diversity$diversity.standardized
region_paper_by_site <- sites_and_region_df$region[seq_len(Nre_audit)]
region_jags_by_site <- paper_region_jags
div_jags_by_site <- diversity_vec[region_jags_by_site]

region_positional_mismatch <-
  region_true_by_site[obs_site] != region_paper_by_site[obs_site]
region_jags_mismatch <-
  region_true_by_site[obs_site] != region_jags_by_site[obs_site]
diversity_true_obs <- div_true_by_site[obs_site]
diversity_jags_obs <- div_jags_by_site[obs_site]
diversity_mismatch <- abs(diversity_true_obs - diversity_jags_obs) > 1e-12

indexing_audit <- data.frame(
  obs_index = seq_len(nrow(data)),
  Reef_ID = obs_reef,
  Ecoregion = obs_ecoregion,
  ERG = if ("ERG" %in% names(data)) as.character(data$ERG) else NA_character_,
  site = obs_site,
  re = obs_site,
  ecoregion_true_for_site = eco_true_by_site[obs_site],
  region_true = region_true_by_site[obs_site],
  region_paper_row_at_site_id = region_paper_by_site[obs_site],
  region_positional_mismatch = region_positional_mismatch,
  region_true = region_true_by_site[obs_site],
  region_jags_index_for_site = region_jags_by_site[obs_site],
  region_jags_mismatch = region_jags_mismatch,
  diversity_true_for_site = diversity_true_obs,
  diversity_jags_for_site = diversity_jags_obs,
  diversity_mismatch = diversity_mismatch,
  stringsAsFactors = FALSE
)
# Repeat region_true header so each mismatch is preceded by its comparison pair.
names(indexing_audit)[11] <- "region_true"

write.csv(
  indexing_audit,
  file = file.path(output_directory, "jags_indexing_audit_by_observation.csv"),
  row.names = FALSE
)
cat(
  "Saved indexing audit:",
  file.path(output_directory, "jags_indexing_audit_by_observation.csv"),
  sprintf(
    "(%d obs; region positional mismatch on %d site-ids; JAGS region mismatch on %d site-ids)\n",
    nrow(indexing_audit),
    sum(region_true_by_site != region_paper_by_site, na.rm = TRUE),
    sum(region_true_by_site != region_jags_by_site, na.rm = TRUE)
  )
)
```
...
X <- model.matrix(~ lat + Depth + Human_pop + Cyclone + SST_mean + SSTA_Mean + SSTA_min + SSTA_freqstdev + SSTA_dhwmax + TSA_max + TSA_freqstdev + Turbidity_mean + Historical_SST_max, data =X_standardized) #OT – including intercept in leads to convergence issues. But this is not the main issue at play.
...
win.data <- list(Y    = data$coral_cover_Beta,
                 ...
                 # diversity = region_diversity_df$diversity.standardized  #OT – this was my fix
)
...
J0 <- jags.parallel(data = win.data,
           inits = inits,
           parameters = params,
          ...
          #  n.burnin = 4000,
          #  n.iter   = 15000)
           n.burnin = 50, #OT – this has been reduced for debugging
           n.iter   = 100)
