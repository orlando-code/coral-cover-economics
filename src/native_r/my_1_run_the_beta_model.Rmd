...
output_directory <- Sys.getenv("BETA_OUTPUT_DIR", unset = file.path(data_dir, "output"))
...
# Modern shapefile loading using terra
# library(terra)
...
# No need to convert to sp::SpatialPolygons or manipulate slots when using terra.
# 'ECO' is now a SpatVector polygon object and can be used with terra and sf functions.
# For compatibility with downstream code, you may want to ensure attribute names match expectations.
# If any downstream code expects a SpatialPolygonsDataFrame from 'sp', use as.polygons()/as.data.frame() and sf::as_Spatial methods,
# but it is recommended to update analysis to work directly with SpatVector objects.
...
# # Ensure CRS matches between points and polygons (assume ECO is correct)
# if (!terra::crs(coral_cover_points) == terra::crs(ECO)) {
#   coral_cover_points <- terra::project(coral_cover_points, terra::crs(ECO))
# }

# Extract ecoregion attributes from polygons to points
# eco_extracted <- terra::extract(ECO, coral_cover_points)
...
# Optionally, remove "ID" column added by terra::extract if present
if ("ID" %in% colnames(coral_cover_data)) {
  coral_cover_data$ID <- NULL
}

# Use data_for_maps.csv as the trusted reef -> ecoregion/diversity mapping.
...
region_diversity_df <- data %>%
  dplyr::distinct(Ecoregion, diversity.standardized) %>%
  dplyr::mutate(region = as.numeric(as.factor(Ecoregion))) %>%
...
# CHAIN PLOT
chains_plot <- MyBUGSChains(out, vars = beta_params, PanelNames = beta_labels)
png(
  file.path(
    log_root,
...
acf_plot <- MyBUGSACF(
  Output = out,
  SelectedVar = beta_params,
  PanelNames = beta_labels
)
png(
  file.path(
    log_root,
